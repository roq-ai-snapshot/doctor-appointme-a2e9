...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiDatabase } from 'react-icons/fi';

import { DfdfweInterface } from 'interfaces/dfdfwe';
import { getDfdfwes } from 'apiSdk/dfdfwes';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<DfdfweInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/dfdfwes` : null),
    () => getDfdfwes({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Dfdfwes',
      path: isTenantUser ? `/dfdfwes/view/${data?.[0]?.id}` : '/dfdfwes',
      entity: 'dfdfwe',
